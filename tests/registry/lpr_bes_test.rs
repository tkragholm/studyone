...
async fn test_lpr_bes_filter_by_procedure() -> par_reader::Result<()> {
    ...
    let filter_expr = Expr::Eq(proc_column.to_string(), LiteralValue::String("K".to_string()));
    ...
    let complex_filter = Expr::And(vec![
        Expr::Eq(proc_column.to_string(), LiteralValue::String("KF".to_string())), // Example procedure code prefix
        Expr::GtEq(
