//! Integration test for the complete workflow from population generation to matching
...
use crate::utils::{ensure_path_exists, get_available_year_files, registry_dir, test_data_dir};
...
fn load_real_diagnoses(population: &Population) -> Result<DiagnosisCollection> {
    // Check if LPR data path exists
    let lpr_path = registry_dir("lpr");
    ensure_path_exists(&lpr_path)?;
    ...
    info!("Found {} LPR_DIAG files and {} LPR_ADM files to process", 
          lpr_diag_files.len(), lpr_adm_files.len());
    ...
    for (diag_idx, diag_file) in lpr_diag_files.iter().enumerate() {
        ...
        let diag_year = diag_file.file_stem()
            .and_then(|name| name.to_string_lossy().parse::<u32>().ok())
            .unwrap_or(0);
        
        info!("Processing year {} - DIAG: {:?}, ADM: {:?}", 
              diag_year, diag_file.file_name().unwrap_or_default(), 
