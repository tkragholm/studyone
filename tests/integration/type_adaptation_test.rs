use std::sync::Arc;
use arrow::array::{ArrayRef, StringArray, Int32Array, Float64Array, Date32Array, StringBuilder};
use arrow::datatypes::{DataType, Field, Schema};
use arrow::record_batch::RecordBatch;
use par_reader::schema::adapters::{
    check_type_compatibility, TypeCompatibility, check_schema_with_adaptation,
    convert_array, DateFormatConfig, AdaptationStrategy, adapt_record_batch
};
...
fn test_schema_compatibility_report() {
    ...
    // Check strategies
    let id_adaptation = report.adaptations.iter()
        .find(|a| a.field_name == "id")
        .expect("Should have adaptation for id field");
    assert_eq!(id_adaptation.adaptation_strategy, AdaptationStrategy::NumericConversion);
    
    let date_adaptation = report.adaptations.iter()
        .find(|a| a.field_name == "date_str")
        .expect("Should have adaptation for date_str field");
    assert_eq!(date_adaptation.adaptation_strategy, AdaptationStrategy::DateParsing);
}
...
fn test_string_to_date_conversion() {
    ...
    for date in &date_strings {
        builder.append_value(date).unwrap();
    }
...
fn test_numeric_conversion() {
    ...
    // Verify the result
    let float_array = result.as_any().downcast_ref::<Float64Array>()
...
fn test_adapt_record_batch() {
    ...
    let date_array = Arc::new(StringArray::from(vec!["2023-01-15", "2023-02-20", "2023-03-30"]));
    ...
    let adapted_batch = adapt_record_batch(
        &source_batch, 
        &target_schema,
        &date_config
    ).unwrap();
    ...
    assert_eq!(adapted_batch.schema().field(0).data_type(), &DataType::Int64);
    assert_eq!(adapted_batch.schema().field(2).data_type(), &DataType::Date32);
...
fn test_date_format_detection() {
    ...
    let formats = vec![
        // ISO format
        Arc::new(StringArray::from(vec!["2023-01-15", "2023-02-20", "2023-03-30"])),
        // DD/MM/YYYY format
        Arc::new(StringArray::from(vec!["15/01/2023", "20/02/2023", "30/03/2023"])),
        // MM/DD/YYYY format
        Arc::new(StringArray::from(vec!["01/15/2023", "02/20/2023", "03/30/2023"])),
