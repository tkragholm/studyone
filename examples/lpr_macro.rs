//! Example demonstrating the RegistryTrait with LPR v2 data
//!
//! This example shows how to use the procedural macros to define LPR registry types
...
fn read_parquet(path: &Path) -> Result<Vec<RecordBatch>> {
    ...
    let file_reader = SerializedFileReader::new(file).map_err(|e| {
        error::Error::ParquetError(format!("Failed to read parquet file: {}", e))
    })?;
    ...
    for maybe_batch in batch_reader {
        let batch = maybe_batch.map_err(|e| {
            error::Error::ParquetError(format!("Failed to read batch: {}", e))
        })?;
        batches.push(batch);
    }
...
fn main() -> Result<()> {
    println!("Running LPR schema macros example");
    ...
    let adm_parquet_path =
        Path::new("/Users/tobiaskragholm/generated_data/parquet/lpr/adm.parquet");
    let diag_parquet_path =
        Path::new("/Users/tobiaskragholm/generated_data/parquet/lpr/diag.parquet");
