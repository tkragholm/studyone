...
impl UddfRegistry for Individual {
    ...
    fn enhance_with_education_data(&mut self, batch: &RecordBatch, row: usize) -> Result<bool> {
        ...
        if let Some(enhanced) = crate::registry::uddf::deserializer::deserialize_row(batch, row)? {
            ...
            if let Some(valid_from) = enhanced.education_valid_from {
                self.education_valid_from = Some(valid_from);
            }

            if let Some(valid_to) = enhanced.education_valid_to {
                self.education_valid_to = Some(valid_to);
            }
