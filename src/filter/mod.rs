...
impl Expr {
    ...
    fn collect_required_columns(&self, columns: &mut HashSet<String>) {
        match self {
            Self::Eq(col, _) | Self::NotEq(col, _) | Self::Gt(col, _) | 
            Self::GtEq(col, _) | Self::Lt(col, _) | Self::LtEq(col, _) |
            Self::In(col, _) | Self::NotIn(col, _) | Self::IsNull(col) | 
            Self::IsNotNull(col) | Self::Contains(col, _) | Self::StartsWith(col, _) | 
            Self::EndsWith(col, _) => {
...
fn apply_logical_and(left: &BooleanArray, right: &BooleanArray) -> Result<BooleanArray> {
    ...
    for i in 0..left.len() {
        let left_val = if left.is_null(i) { false } else { left.value(i) };
        let right_val = if right.is_null(i) { false } else { right.value(i) };
