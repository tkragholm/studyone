...
impl ParquetLoader {
    ...
    #[must_use] pub const fn with_schema_ref(schema: SchemaRef) -> Self {
        Self {
            schema: Some(schema),
            batch_size: None,
        }
...
impl AsyncLoader for ParquetLoader {
    fn load_async<'a>(
        ...
        Box::pin(async move {
            AsyncFileHelper::read_parquet_file(path, schema, batch_size).await
        })
    }
...
impl AsyncDirectoryLoader for ParquetLoader {
    fn find_files_async<'a>(
        ...
        Box::pin(async move {
            AsyncFileHelper::find_parquet_files(dir).await
        })
    }
}
...
impl PnrFilterableLoader {
    ...
    #[must_use] pub const fn with_pnr_column(mut self, column_name: &'static str) -> Self {
