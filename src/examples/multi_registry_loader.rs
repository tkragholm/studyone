...
use crate::registry::{
    akm, bef, ind, mfr, uddf, vnds, deserializer::create_mapped_batch
};
...
impl RegistryType {
    ...
    #[must_use] pub fn common_columns() -> Vec<String> {
        vec![
    ...
    #[must_use] pub fn specific_columns(&self) -> Vec<String> {
        match self {
            Self::BEF => vec![
                "FAR_ID".to_string(),
                "MOR_ID".to_string(),
...
pub async fn load_multi_registry_data(
    ...
    for registry_type in registry_types {
        ...
        info!("Loading {} data from {}", registry_name, registry_dir.display());
        ...
        let record_batches = match load_parquet_files_parallel_with_filter(
            ...
            Ok(batches) => {
                info!("Loaded {} record batches from {}", batches.len(), registry_name);
                stats.insert(format!("{registry_name}_batches"), batches.len());
                
                let total_rows = batches.iter()
        ...
        let (individuals, filtered_stats) = filter_and_convert_registry(
            record_batches,
            registry_type,
            date_start,
            date_end,
...
fn filter_and_convert_registry(
    ...
    for batch in &record_batches {
            ...
            .downcast_ref::<arrow::array::StringArray>()
        {
            ...
            if !indices.is_empty() {
                ...
                let filtered_batch = take::take_record_batch(
                    batch, 
                    &arrow::array::UInt64Array::from(indices)
                )?;
    ...
    for (batch_idx, batch) in filtered_batches.iter().enumerate() {
        ...
        let mapped_batch = match create_mapped_batch(batch, field_mapping) {
            ...
            Err(e) => {
                ...
                if seen_errors.insert(error_msg.clone()) {
                    ...
                    warn!("[{}] Failed to map batch {}: {}", 
                        registry_type.as_str(), batch_idx, error_msg);
                }
                error_count += 1;
                continue;
            }
        ...
        match SerdeIndividual::from_batch(&mapped_batch) {
            ...
            Err(e) => {
                ...
                if seen_errors.insert(error_msg.clone()) {
                    // Only log unique errors
                    warn!("[{}] Failed to deserialize batch {}: {}", 
                        registry_type.as_str(), batch_idx, error_msg);
                }
                error_count += 1;
            }
    ...
    if error_count > 0 {
        warn!(
            ...
            registry_type.as_str(), error_count, seen_errors.len()
        );
        ...
        if seen_errors.len() > 1 {
            ...
            if seen_errors.len() > 3 {
                warn!("[{}]   ... and {} more error types", 
                    registry_type.as_str(), seen_errors.len() - 3);
            }
        }
    }
...
pub async fn run_multi_registry_example(base_dir: &Path) -> Result<usize> {
    ...
    info!("Loading data from multiple registry types at {}", base_dir.display());
    info!("Filter: birth dates between {date_start} and {date_end}");
    
    let collection = load_multi_registry_data(
        base_dir,
        &registry_types,
        date_start,
        date_end,
    ...
    let total_individuals = collection.stats.get("total_individuals").copied().unwrap_or(0);
    ...
    for registry_type in &registry_types {
        let registry_name = registry_type.as_str();
        let success_count = collection.stats.get(&format!("{registry_name}_success_count")).copied().unwrap_or(0);
        let total_rows = collection.stats.get(&format!("{registry_name}_total_rows")).copied().unwrap_or(0);
        
        if total_rows > 0 {
            println!("{registry_name} Registry:");
            println!("  Total rows: {total_rows}");
            println!("  Individuals in date range: {success_count}");
            println!("  Filtering rate: {:.2}%", (success_count as f64 / total_rows as f64) * 100.0);
        } else if collection.stats.get(&format!("{registry_name}_skipped")).is_some() {
            println!("{registry_name} Registry: Directory not found, skipped");
        } else if collection.stats.get(&format!("{registry_name}_error")).is_some() {
